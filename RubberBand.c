...
#ifndef lint
static char *rcsid_RubberBand_c = "$Header: /usr/graph2/X11.3/contrib/windowmgrs/awm/RCS/RubberBand.c,v 1.2 89/02/07 21:23:46 jkh Exp $";
#endif  lint
...
Window transient;
{
     ...
     unsigned int popw, poph;        /* width and height of prompt window*/
#define invertGC DrawGC
     ...
     unsigned int events;               /* what events we want.             */
     ...
     while (1) {
	  if (XGrabPointer (dpy, RootWindow(dpy, scr), FALSE, events, 
				   GrabModeAsync, GrabModeAsync, None, ul,
     ...
     nz += 9;
     popw = XTextWidth (PFontInfo, text, nz) + 2 * IBW;
     poph = PFontInfo->ascent+PFontInfo->descent + 2 * IBW;
     
     pop = XCreateSimpleWindow(dpy, RootWindow(dpy, scr), 
			       0, 0, popw, poph, BW, PBorder, PBackground);
     XMapWindow (dpy, pop);
     ...
     while (stop == FALSE) {
	  ...
	  if (changed) {
	       ...
	       XDrawImageString(dpy, pop, PopGC, IBW, IBW+PFontInfo->ascent,
				text, nz);
	  }
	  if (doit) {
